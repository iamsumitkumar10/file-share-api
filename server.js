...
app.post('/upload', authMiddleware, upload.single('file'), async (req, res) => {
  try {
    ...
    // ✅ remove both original and encrypted files locally in uploads/
    fs.unlinkSync(req.file.path); // removed
    fs.unlinkSync(path);          // removed

    res.json({
      message: 'File uploaded securely',
      key,
      iv,
      originalFile: req.file.path,   // path to original file in uploads/
      encryptedFile: path,           // path to encrypted file in uploads/
      filename: req.file.originalname + '.enc',
    });
  } catch (err) {
    console.error('Upload error:', err);
